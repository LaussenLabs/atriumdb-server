...
class Config:
    def __init__(self):
        ...
        if hasattr(self, svc_tsc_gen_cfg['metadb_connection']) and "type" in getattr(self, svc_tsc_gen_cfg['metadb_connection']):
            # get metadata database connection
            svc_tsc_gen_cfg['metadb_connection'] = getattr(self, svc_tsc_gen_cfg['metadb_connection'])
            if 'username' not in svc_tsc_gen_cfg['metadb_connection'] or 'password' not in svc_tsc_gen_cfg['metadb_connection']:
            ...
            raise ConfigurationError(f"Metadata connection name '{svc_tsc_gen_cfg['metadb_connection']}' not found in config file. "
                             f"For an example see example_config.yaml under the header 'metadb'. Replace 'metadb' with "
                             f"'{svc_tsc_gen_cfg['metadb_connection']}' and fill fields with your connection info.")


        svc_tsc_gen_cfg['max_workers'] = None if svc_tsc_gen_cfg['max_workers'] == 'None' else int(svc_tsc_gen_cfg['max_workers'])

        # set connection parameters if the database type is not sqlite
        if svc_tsc_gen_cfg['metadb_connection']['type'] == "sqlite":
            self.CONNECTION_PARAMS = None
        else:
            self.CONNECTION_PARAMS = {'host': svc_tsc_gen_cfg['metadb_connection']['host'],
                                      'user': svc_tsc_gen_cfg['metadb_connection']['username'],
                                      'password': svc_tsc_gen_cfg['metadb_connection']['password'],
                                      'database': svc_tsc_gen_cfg['metadb_connection']['db_name'],
                                      'port': svc_tsc_gen_cfg['metadb_connection']['port']}
